...
* consumes only the stored execution-v3 output tables and does not require
  per-run prediction arrays,
...
def test_runs_from_summary_tables_without_raw_predictions(analysis) -> None:
...
def test_skipped_regime_diagnostics_are_explicit(analysis) -> None:
    ...
    # The skip must not be silent: it appears in the interpretation statuses too.
...
def test_report_has_no_forbidden_claims(analysis) -> None:
    ...
    # These unsupported phrases must never appear as findings, even negated.
    never_present = (
    ...
    assert alpha not in text or "does not estimate" in text
