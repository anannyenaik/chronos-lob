...
| fi2010_ssl_v2_benchmark | partial_real | archived | no | 6 | 0 | Recorded git commit is older than the current repository commit, but retained hashes, files and summaries are consistent; the artefact is summary-valid at its generating commit. |
| ssl_v2_analysis_report | partial_real | fresh | no |  | 0 | Hash/commit staleness checks passed. |
...
| fi2010_ssl_v2_benchmark | partial_real | 84aa70ebef89 | Recorded git commit is older than the current repository commit, but retained hashes, files and summaries are consistent; the artefact is summary-valid at its generating commit. |
...
| binance_l2_extension_report | partial_real | 8d69fbb00a10 | Recorded git commit is older than the current repository commit, but retained hashes, files and summaries are consistent; the artefact is summary-valid at its generating commit. |
