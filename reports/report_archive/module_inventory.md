# Module Inventory

Public modules are grouped by package area to support architecture and implementation references in the final technical report.

## `data`

- `chronoslob.data.binance`
- `chronoslob.data.event_store`
- `chronoslob.data.fi2010`
- `chronoslob.data.manifests`
- `chronoslob.data.schemas`
- `chronoslob.data.validation`

## `book`

- `chronoslob.book.event_replay`
- `chronoslob.book.events`
- `chronoslob.book.local_order_book`
- `chronoslob.book.reconstruction`
- `chronoslob.book.replay`

## `features`

- `chronoslob.features.imbalance`
- `chronoslob.features.microprice`
- `chronoslob.features.order_flow`
- `chronoslob.features.pipeline`
- `chronoslob.features.regimes`
- `chronoslob.features.volatility`

## `labels`

- `chronoslob.labels.adverse_selection`
- `chronoslob.labels.fill_probability`
- `chronoslob.labels.leakage`
- `chronoslob.labels.midprice`
- `chronoslob.labels.pipeline`
- `chronoslob.labels.spread`
- `chronoslob.labels.volatility`

## `training`

- `chronoslob.training.artifacts`
- `chronoslob.training.baseline_experiment`
- `chronoslob.training.batching`
- `chronoslob.training.calibration`
- `chronoslob.training.config`
- `chronoslob.training.dataloaders`
- `chronoslob.training.datasets`
- `chronoslob.training.evaluate`
- `chronoslob.training.experiment`
- `chronoslob.training.metrics`
- `chronoslob.training.multitask_datasets`
- `chronoslob.training.multitask_experiment`
- `chronoslob.training.splitters`
- `chronoslob.training.ssl_datasets`
- `chronoslob.training.ssl_experiment`
- `chronoslob.training.token_batching`
- `chronoslob.training.token_datasets`
- `chronoslob.training.torch_experiment`
- `chronoslob.training.torch_training`
- `chronoslob.training.transformer_experiment`

## `models`

- `chronoslob.models.baselines`
- `chronoslob.models.calibration`
- `chronoslob.models.deeplob`
- `chronoslob.models.multitask`
- `chronoslob.models.preprocessing`
- `chronoslob.models.ssl`
- `chronoslob.models.tokenisation`
- `chronoslob.models.transformer`

## `backtest`

- `chronoslob.backtest.costs`
- `chronoslob.backtest.execution`
- `chronoslob.backtest.latency`
- `chronoslob.backtest.risk`
- `chronoslob.backtest.turnover`
- `chronoslob.backtest.validation`

## `analysis`

- `chronoslob.analysis.ablations`
- `chronoslob.analysis.regimes`
- `chronoslob.analysis.sensitivity`
- `chronoslob.analysis.summary`
- `chronoslob.analysis.transfer`

## `utils`

- `chronoslob.utils.audit`
- `chronoslob.utils.logging`
- `chronoslob.utils.paths`
- `chronoslob.utils.report_archive`
- `chronoslob.utils.seeding`
