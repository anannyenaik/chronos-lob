"""Training loop and checkpointing for matrix SSL-v2 pretraining."""
...
from chronoslob.models.matrix_ssl_v2 import (
    MatrixSSLV2Config,
    MatrixSSLV2Model,
...
__all__ = [
    "MATRIX_SSL_V2_CHECKPOINT_VERSION",
    "MatrixSSLV2EpochResult",
    "MatrixSSLV2TrainingConfig",
    "evaluate_matrix_ssl_v2",
    "fit_matrix_ssl_v2",
    "load_pretrained_ssl_v2_encoder_state",
    "save_pretrained_ssl_v2_encoder",
    "train_matrix_ssl_v2_one_epoch",
]

MATRIX_SSL_V2_CHECKPOINT_VERSION = "fi2010-matrix-ssl-v2/encoder/v1"


def _require_torch() -> Any:
    if not _TORCH_AVAILABLE:
        raise ImportError(
            "PyTorch is required for the matrix SSL-v2 experiment. Install the "
...
def _resolve_device(device: str) -> Any:
    ...
    raise ValueError("matrix SSL-v2 training supports 'cpu' or 'cuda'-prefixed devices")


@dataclass(frozen=True)
class MatrixSSLV2TrainingConfig:
    """Configuration for the SSL-v2 pretraining loop."""
...
@dataclass(frozen=True)
class MatrixSSLV2EpochResult:
    """Per-epoch SSL-v2 train/validation losses."""
...
def _move_batch_to_device(batch: Mapping[str, Any], device_obj: Any) -> dict[str, Any]:
    ...
    for key, value in batch.items():
        if isinstance(value, Mapping):
            moved[key] = {
                str(child_key): child_value.to(device_obj)
                if torch_module.is_tensor(child_value)
                else child_value
...
def _assert_finite_gradients(model: Any) -> None:
    ...
    for name, parameter in model.named_parameters():
        ...
        if torch_module.isinf(parameter.grad).any().item():
            raise RuntimeError(f"gradient for parameter {name!r} contains infinite values")


def _forward_batch(model: MatrixSSLV2Model, batch: Mapping[str, Any]) -> Any:
    return model(
        batch["x"],
        mask=batch.get("mask"),
        masked_target=batch.get("masked_target"),
        future_labels=batch.get("future_labels"),
...
def train_matrix_ssl_v2_one_epoch(
    model: MatrixSSLV2Model,
    ...
    *,
    ...
    """Run one SSL-v2 pretraining epoch."""
    torch_module = _require_torch()
    if model is None:
    ...
    for raw_batch in dataloader:
        ...
        if output.loss is None:
            raise RuntimeError("matrix SSL-v2 forward did not produce a loss")
        if not torch_module.isfinite(output.loss).item():
            raise RuntimeError("matrix SSL-v2 training loss became non-finite")
    ...
    if total_samples == 0:
        raise ValueError("train_matrix_ssl_v2_one_epoch received an empty dataloader")
    return total_loss / total_samples, _average_components(
        component_totals,
        component_counts,
    )


def evaluate_matrix_ssl_v2(
    model: MatrixSSLV2Model,
    dataloader: Iterable[Any],
    *,
    device: str = "cpu",
) -> dict[str, Any]:
    """Evaluate an SSL-v2 model without updating parameters."""
    torch_module = _require_torch()
    if model is None:
        raise ValueError("model must not be None")
    device_obj = _resolve_device(device)
    ...
    with torch_module.no_grad():
        for raw_batch in dataloader:
            ...
            output = _forward_batch(model, batch)
            if output.loss is None:
                raise RuntimeError("matrix SSL-v2 evaluation forward produced no loss")
            if not torch_module.isfinite(output.loss).item():
                raise RuntimeError("matrix SSL-v2 evaluation loss became non-finite")
            total_loss += float(output.loss.item()) * batch_size
            total_samples += batch_size
            _accumulate_components(
                component_totals,
    ...
    if total_samples == 0:
        raise ValueError("evaluate_matrix_ssl_v2 received an empty dataloader")
    return {
...
def fit_matrix_ssl_v2(
    model: MatrixSSLV2Model,
    train_loader: Iterable[Any],
    validation_loader: Iterable[Any] | None = None,
    config: MatrixSSLV2TrainingConfig | None = None,
) -> list[MatrixSSLV2EpochResult]:
    """Pretrain an SSL-v2 model and return per-epoch results."""
    _require_torch()
    training_config = config if config is not None else MatrixSSLV2TrainingConfig()
    if not isinstance(training_config, MatrixSSLV2TrainingConfig):
        raise TypeError("config must be a MatrixSSLV2TrainingConfig instance")
    ...
    history: list[MatrixSSLV2EpochResult] = []
    for epoch in range(1, training_config.epochs + 1):
        train_loss, train_components = train_matrix_ssl_v2_one_epoch(
            model,
            train_loader,
            optimizer,
        ...
        if validation_loader is not None:
            evaluation = evaluate_matrix_ssl_v2(
                model,
                validation_loader,
                device=training_config.device,
            )
...
def save_pretrained_ssl_v2_encoder(
    model: MatrixSSLV2Model,
    path: str | Path,
    *,
    metadata: Mapping[str, Any] | None = None,
) -> Path:
    """Save transferable SSL-v2 encoder weights and architecture metadata."""
    torch_module = _require_torch()
    if not isinstance(model, MatrixSSLV2Model):
        raise TypeError("model must be a MatrixSSLV2Model instance")
    resolved = Path(path)
    resolved.parent.mkdir(parents=True, exist_ok=True)
    config = model.config
    payload: dict[str, Any] = {
        "checkpoint_version": MATRIX_SSL_V2_CHECKPOINT_VERSION,
        ...
        "ssl_v2_objectives": list(config.enabled_objectives()),
...
def load_pretrained_ssl_v2_encoder_state(path: str | Path) -> dict[str, Any]:
    """Load an encoder state dict saved by :func:`save_pretrained_ssl_v2_encoder`."""
    torch_module = _require_torch()
    resolved = Path(path)
    if not resolved.is_file():
        raise FileNotFoundError(f"pretrained SSL-v2 checkpoint not found: {resolved}")
    ...
    if not isinstance(payload, Mapping) or "encoder_state_dict" not in payload:
        raise ValueError("checkpoint does not contain an 'encoder_state_dict'")
...
_RESERVED_MODEL_FACTORY = create_matrix_ssl_v2_model
_RESERVED_CONFIG = MatrixSSLV2Config
