...
This module combines the tokeniser, transformer encoder and SSL wrapper in a
small, deterministic training and evaluation path. It includes:
...
precision or checkpointing. The smoke runner marks the produced losses as
synthetic smoke-test diagnostics.
"""
...
_SYNTHETIC_SSL_WARNING = (
    "Synthetic SSL smoke test; losses describe implementation behaviour and do not "
    "represent market signal or benchmark performance."
)
...
def run_ssl_smoke_from_event_log(
    ...
    *,
    ...
    The function tokenises the event log with the standard defaults, builds
    ...
    explicit ``notes`` field identifying the run as a synthetic smoke test.
    """
    _require_torch()
    file_path = Path(path)
