...
def create_baseline_model(config: BaselineModelConfig) -> BaseBaselineModel:
    ...
    if config.model_type == "elastic_net_logistic":
        ...
        params["solver"] = "saga"
        # Since scikit-learn 1.8, l1_ratio selects elastic-net regularisation
        # and the penalty argument is deprecated. Older supported releases
        # still require the explicit penalty.
        logistic_parameters = LogisticRegression().get_params()
        if "penalty" in logistic_parameters and logistic_parameters["penalty"] != "deprecated":
