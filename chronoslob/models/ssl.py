...
The :class:`MarketTransformerEncoder` is wrapped with self-supervised
objectives over field-wise token batches. The supported objectives are:
    ...
    def __post_init__(self) -> None:
        ...
        if self.enable_contrastive_loss:
            raise NotImplementedError(
                "The contrastive SSL objective is not implemented; set "
...
class MarketSSLTransformer(_TORCH_MODULE_BASE):
    ...
    def forward(
        ...
        *,
        ...
        ``attention_mask``, matching the encoder contract. When
...
def create_ssl_transformer(config: SSLTransformerConfig) -> MarketSSLTransformer:
    ...
    modelling and next-field prediction. The contrastive objective is not
    implemented.
    """
