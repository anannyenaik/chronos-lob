...
Supervised multi-task fine-tuning builds on the tokenisation layer
(:mod:`chronoslob.models.tokenisation`) and transformer encoder
(:mod:`chronoslob.models.transformer`). It exposes one
...
proxy. All supported tasks are classification tasks. Binary tasks are
...
class TaskType:
    ...
    The class exposes string constants for the supported classification heads.
    """
...
class MultiTaskTransformer(_TORCH_MODULE_BASE):
    ...
    def forward(
        ...
        *,
        ...
        attention mask, matching the encoder contract. ``targets``,
...
def create_multitask_transformer(
    ...
    The returned model wraps a transformer encoder backbone with
    one classification head per configured task. It implements supervised
    multi-task fine-tuning only.
    """
