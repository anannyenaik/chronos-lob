...
``torch.nn.TransformerEncoder`` and a supervised classification head. It
contains no self-supervised objectives, masked event modelling, next-event
prediction, calibration or execution simulation.
...
# Derive the canonical token-field names directly from the token enum so
...
class MarketTransformerEncoder(_TORCH_MODULE_BASE):
    ...
    def forward(
        ...
        *,
        ...
        ``attention_mask`` (True marks real tokens). The tokenisation attention
        ...
        # src_key_padding_mask; ChronosLOB attention masks use True = real.
...
def create_market_transformer(
    ...
    implement a self-supervised objective. Benchmark results require fitting
    and evaluating it through an experiment runner.
    """
