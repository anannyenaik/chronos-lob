"""Summary analysis for the validation-selected neural benchmark."""
...
def _successful_results(source: Path) -> pd.DataFrame:
    ...
    if successful.empty:
        raise ValueError("neural benchmark analysis requires completed rows")
    return successful
...
def analyse_proper_neural_benchmark(
    ...
    *,
    ...
    """Build compact summaries while raw per-run predictions are available."""
    source = Path(source_dir)
    if not source.is_dir():
        raise FileNotFoundError(f"neural benchmark directory is missing: {source}")
    ...
    payload = {
        "analysis": "validation-selected neural benchmark",
        ...
        "storage_policy": (
            "stored CSV and JSON summaries are compact; per-run predictions, "
