...
| `build-final-empirical-report --classical PATH --neural PATH --uncertainty PATH [--ablations PATH] [--feature-ablations PATH] [--feature-ablation-analysis PATH] [--execution PATH] [--execution-v3 PATH] [--external PATH] [--synthetic-lob PATH] [--ssl PATH] [--neural-full-grid PATH] [--evidence-pack PATH] --out PATH [--overwrite]` | Build the final FI-2010 empirical report and summary JSON from stored multi-fold, uncertainty, ablation, execution-proxy and external-context artefacts. With feature-ablation analysis and evidence-pack inputs, the report includes scoped stability and claim-audit status. The optional synthetic-lob input adds a clearly labelled synthetic event-level extension section. |
| `build-evidence-pack --out PATH --neural-full-grid PATH --figures PATH --execution-v3 PATH --feature-ablations PATH [--feature-ablation-analysis PATH] --ablation-figures PATH --final-report PATH [--strict\|--no-strict] [--allow-smoke-test] [--overwrite\|--no-overwrite]` | Build the release evidence pack: artefact inventory, claim audit, conservative README snapshot, public bullet files, reproduction commands and release checklist. Smoke artefacts remain labelled as diagnostics and are not empirical evidence. See [EVIDENCE_PACK.md](EVIDENCE_PACK.md). |
