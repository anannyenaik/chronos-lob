...
| `run-fi2010-feature-ablations [--config PATH] [--processed-root PATH] [--data-path PATH] --folds LIST --horizons LIST --seeds LIST --models LIST --feature-groups LIST --ablation-modes LIST --out PATH [--res<!-- -->ume\|--no-res<!-- -->ume] [--strict\|--no-strict] [--smoke-test]` | Run classical FI-2010 microstructure feature ablations, write per-run configs/metrics/predictions/status files and root `results_summary.csv`, `aggregate_summary.csv` and `feature_delta_summary.csv`. Snapshot-flow columns are labelled proxies, not true OFI. See [FEATURE_ABLATIONS.md](FEATURE_ABLATIONS.md). |
...
| `build-final-empirical-report --classical PATH --neural PATH --uncertainty PATH [--ablations PATH] [--feature-ablations PATH] [--execution PATH] [--execution-v3 PATH] [--external PATH] [--ssl PATH] [--neural-full-grid PATH] [--evidence-pack PATH] --out PATH [--overwrite]` | Build the final FI-2010 empirical report and summary JSON from stored multi-fold, uncertainty, ablation, execution-proxy and external-context artefacts. With `--evidence-pack`, the report includes claim-audit status and release caveats. |
| `build-evidence-pack --out PATH --neural-full-grid PATH --figures PATH --execution-v3 PATH --feature-ablations PATH --ablation-figures PATH --final-report PATH [--strict\|--no-strict] [--allow-smoke-test] [--overwrite\|--no-overwrite]` | Build the release evidence pack: artefact inventory, claim audit, conservative README snapshot, public bullet files, reproduction commands and release checklist. Smoke artefacts remain labelled as diagnostics and are not empirical evidence. See [EVIDENCE_PACK.md](EVIDENCE_PACK.md). |
